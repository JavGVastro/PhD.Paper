...
for mingood in zip(mingoods):
 [sb], m= downsample([sb], m, weights=None, mingood=mingood)
...
for mingood in zip(mingoods):
 [vv], m= downsample([vv], m, weights=None, mingood=mingood)
...
for mingood in zip(mingoods):
 [ss], m= downsample([ss], m, weights=None, mingood=mingood)
...
data_export_matrix = {
       ...
       's0' : s0,
       'pix' : pix,
       'sb' : np.array(pd.DataFrame(sb)),
       'vv' : np.array(pd.DataFrame(vv)),
       'ss' : np.array(pd.DataFrame(ss))
...
get_ipython().system('jupyter nbconvert --to script --no-prompt otv-MUSE-N346-S-downsample.ipynb')
