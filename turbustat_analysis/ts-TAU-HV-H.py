...
s0 = data["results_2sig"]['s0'][0] + data["results_2sig"]['s0'][1] #pc
...
axx.annotate('r$_0$', xy=(r0, 2),  xycoords='data',
           xytext=(0.55, 0.95), textcoords='axes fraction',
            arrowprops=dict(facecolor='black', shrink=0.02),
            horizontalalignment='right', verticalalignment='top',
            )

axx.annotate('s$_0$', xy=(s0, 2),  xycoords='data',
...
# sb = fits.open(datapath_obs / 'TAURUS-HX-Ha-Flux.fits')[0].data.astype(float)
# mf = ~np.isfinite(sb) | (sb < 0.0)
# sb[mf] = 0.0
# sb /= sb.max()
# sb /= np.nanmean(sb)
...
get_ipython().system('jupyter nbconvert --to script --no-prompt ts-TAU-HV-H.ipynb')
